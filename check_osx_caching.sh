...
#   v1.3 - 20150330 modded by James Murty to make it work with Yosemite
...
#   10.10+ Server with Server.app 4+
...
# Check to see if we're running Yosemite Server as there's a bit more usage verbosity
...
then
    ...
    yosemitePerfData="macAppsUsage=$macAppsUsage; iosAppsUsage=$iosAppsUsage; ibooksUsage=$ibooksUsage; moviesUsage=$moviesUsage; musicUsage=$musicUsage; otherUsage=$otherUsage;"
fi
...
printf "OK - Caching service appears to be running OK. | reservedSpace=$reservedSpace; cacheUsed=$cacheUsed; cacheFree=$cacheFree; cacheLimit=$cacheLimit; bytesRequested=$bytesRequested; bytesReturned=$bytesReturned; numberOfPkgs=$numberOfPkgs; $yosemitePerfData\n"
exit 0
